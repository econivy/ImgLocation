...
namespace ImgLocation.Services
{
    public class ProcessHelper
    {
        ...
        /// <summary>/// 连续运行模式，支持打开某程序后，持续向其输入命令，直到结束。
        /// </summary>/// <param name="exePath"></param>
        /// <param name="args"></param>
        /// <param name="moreArgs"></param>
        /// <returns></returns>
        /// 
        public static ProcessRunResult RunAsContinueMode(string exePath, string args, string[] moreArgs)
        {
            ...
            try
            {
                using (var p = GetProcess())
                {
                    ...
                    //先输出一个换行，以便将程序的第一行输出显示出来。//如adb.exe，假如不调用此函数的话，第一行等待的shell@android:/ $必须等待下一个命令输入才会显示。
                    ...
                    // Do not wait for the child process to exit before// reading to the end of its redirected stream.// p.WaitForExit();// Read the output stream first and then wait.
                    p.WaitForExit();
            ...
            catch (Win32Exception ex)
            {
                ...
                //System Error Codes (Windows)//http://msdn.microsoft.com/en-us/library/ms681382(v=vs.85).aspx
        ...
        public static ProcessRunResult Run(string exePath, string args)
        {
            var result = new ProcessRunResult();
            try
            {
